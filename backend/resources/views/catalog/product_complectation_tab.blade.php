...
>
    @if($product->complectation->count() > 0)
        <ul class="nav nav-tabs">
            ...
            $active = true;
            /**
             * @var \App\Models\Product\ProductComplectation $complectation
             */
            ?>
            @foreach($product->complectation->items() as $complectation)
                <li class="{{ $active ? 'active' : '' }}">
                    <a href="#tab-complectation-{{ $complectation->id }}" data-toggle="tab">
                        {{ $complectation->name }}
                    </a>
                </li>
                <?php
        ...
        <div class="tab-content">
            ...
            @foreach($product->complectation->items() as $complectation)
                <div
                        class="addComplectionCategoryContainer tab-pane {{ $active ? 'active' : '' }}"
                        id="tab-complectation-{{ $complectation->id }}"
                >
